...
/*   Updated: 2022/01/27 03:14:17 by dselmy           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	read_token(t_token *cur_token, t_data *all)
{
	int		i;
	int		cmd_i;
	int		quoted_flag;
	...
	while (cur_token->token[i])
	{
		if (manage_quotes(cur_token->token[i], &quoted_flag))
			...
			if (manage_redirections(&i, cur_token, all) < 0)
				return (i);	
	}
	return (0);
}
...
int	parse_token(t_data *all)
{
	...
	while (tmp)
	{
		...
		if (!((t_token *)(tmp->content))->cmd)
			error_exit(all);
		err_index = read_token((t_token *)tmp->content, all);
		if (err_index)
